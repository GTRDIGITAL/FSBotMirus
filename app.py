...
def FS_process():
	...
	if request.method == 'POST':
		...
		if(option2==0):
			...
			test2.cell(row=10,column=8).value='''_xlfn.=IF(SUMIF(D:D,641,J:J)>0,IF(OR('3. F30'!F44,'3. F30'!F45=""),"Please, fill F30 ",0),"OK")'''
			...
			test2.cell(row=10,column=7).value="Check Acc 641 with number of employees"
			test2.cell(row=10,column=7).font=fontRed
			test2.cell(row=10,column=8).value='''_xlfn.=IF(SUMIF(D:D,641,J:J)>0,IF(OR('3. F30'!D47,'3. F30'!D48=""),"Please, fill F30 ",0),"OK")'''
			test2.cell(row=10,column=8).font=fontRed
